...
//     // passport local 에서 done을 콜백함수로 생각, 
//     if(err){
//         console.error(err);
//         next(err);
//     }
...
router.post('/:userId/friend',isLoggedIn,async (req,res,next)=> {
  ...
  try{
    const user = await User.findOne({where:{id:req.params.userId}});  //user에는 게시물 작성자의 정보가 들어잇껬지
    console.log(user);
    if(!user){
      res.status(403).send('친구요청을 할 수 없습니다. 존재하지 않는 사용자 입니다.')
    }
